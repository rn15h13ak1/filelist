...
`exclude` パターンに **フォルダ名** が一致した場合、そのフォルダは items に「除外」フラグ付きで残り、配下は走査されません（ファイルが除外パターンに一致した場合は従来通り完全に出力から消えます）。

- ツリービュー: 名前が灰色イタリック、info に `· 除外: <pattern>` 表示
- テーブルビュー: 種別列が「除外」、灰色イタリック
- 詳細モーダル: 「走査状態」行に `除外パターン "X" により配下のフォルダ・ファイルは走査されていません`
- 既定では非表示。ヘッダーの「除外も表示」チェックボックスで切替（`#excluded=1` で URL ハッシュにも反映）
